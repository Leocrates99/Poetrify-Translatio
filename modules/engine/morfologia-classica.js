/**
 * @module engine/morfologia-classica
 *
 * MOTORE MORFOLOGICO · fonte unica per latino e greco.
 *
 * Nasce da un'unificazione: il parse dei lemmi e la costruzione dei paradigmi
 * esistevano in DUE copie — qui (dentro `paradigm.js`) e dentro
 * `translator.html`. Le due copie erano gia' divergenti, e quella del monolite
 * era STALE su forme reali:
 *   · l'infinito futuro latino era `-urus esse` invece di `-urum esse` (il
 *     participio futuro nell'infinitiva regge l'accusativo), sbagliato in
 *     quattro punti fra i regolari e gli irregolari `sum` / `fero` / `eo`;
 *   · la regex degli apofonici della III declinazione greca cercava
 *     `^(pat|met|thygat|and|gast)$` mentre il tema si ricava dal genitivo a
 *     grado zero (metros -> metr): non poteva combaciare mai, e pater, meter,
 *     thygater, aner, gaster finivano fra i regolari;
 *   · mancava l'intera macchina dell'ACCENTO PERSISTENTE
 *     (`_grAccentRead`, `_grAccentThirdForm`, `_grSyllAnalyze`).
 *
 * Questo modulo contiene la versione CORRETTA. Chi la consuma:
 *   · `engine/paradigm.js`, che importa da qui e resta il livello di
 *     PRESENTAZIONE (citazioni, etichette, tabelle HTML) del dizionario;
 *   · `translator.html`, che la riceve nel proprio blocco module e la installa
 *     sui globali usati dal suo pannello dei paradigmi.
 *
 * Regola: qui dentro non entra rendering. La morfologia dice le FORME; come si
 * mostrino e' affare di chi la usa.
 */

export function _grStrip(s) { return (s || '').normalize('NFD').replace(/[̀-ͯ]/g, '').toLowerCase(); }

export function _stripGreekTone(s) {
  if (!s) return s;
  // U+0300 grave, U+0301 acuto, U+0342 perispomeni (circonflesso greco)
  return s.normalize('NFD').replace(/[̀́͂]/g, '').normalize('NFC');
}

export function _fixDoubleAccent(form) {
...
export function _fixGreekParadigmAccents(par) {
...
export function _contractAlpha(ending) {
...
export function _contractEpsilon(ending) {
  const key = _grStrip(ending);
  const map = {
...
export function _contractOmicron(ending) {
  const key = _grStrip(ending);
  const map = {
...
export const _GR_AUG_VOWEL_MAP = {
  'α':'η','ε':'η','η':'η','ο':'ω','ω':'ω','ι':'ι','υ':'υ',
  'αι':'ῃ','ει':'ῃ','οι':'ῳ','αυ':'ηυ','ευ':'ηυ'
};

export function _greekAugment(stem) {
  if (!stem) return stem;
...
export function _placeRecessiveAccent(form) {
...
export function _addAugment(stem) {
  return _greekAugment(stem);
}

export function parseLatinLemma(lemma, pos) {
...
export function buildNounParadigm(parsed) {
...
export function buildAdjParadigm(parsed) {
  if (!parsed) return null;
...
export function buildVerbParadigm(parsed) {
...
export function buildIrregularVerbParadigm(parsed) {
...
export function parseGreekLemma(lemma, pos) {
  if (!lemma) return null;
...
export const _GR_SYL_RE = /(αι|αυ|ει|ευ|οι|ου|υι|ηυ|ωυ|[αεηιουω])/i;

export function _grSyllAnalyze(form) {
  const nfd = (form || '').normalize('NFD');
  let noMark = '';
  const nfdIdx = [];
  for (let i = 0; i < nfd.length; i++) {
...
export function _grAccentRead(form) {
...
export function _grPlaceMark(bareForm, d, mark) {
...
export function _grPenultLong(bareInfo, nomInfo) {
...
export function _grAccentNominalForm(bare, nomInfo, ultimaLong, gd) {
...
export function _grAccentThirdForm(bare, kStart, nomInfo, ultimaLong) {
  const info = _grSyllAnalyze(bare);
  const total = info.total;
  if (total < 1) return bare;
  if (total < 2) {                                   // monosillabo: acuto/circonflesso semplice
...
export function _grRecessiveNominal(bare) {
  const info = _grSyllAnalyze(bare);
  const total = info.total;
  if (total < 1) return bare;
  if (total < 2) {
...
export const _GR_PLUR_I = {                                  // plurale comune della I declinazione
  Nominativo: ['αι', 0], Genitivo: ['ων', 1], Dativo: ['αις', 1], Accusativo: ['ας', 1], Vocativo: ['αι', 0]
};

export const _GR_ENDINGS = {
  'II-m': {
...
export function _declineGreekNominal(stemBare, nomInfo, endingKey, opts) {
  opts = opts || {};
...
export function buildGreekNounParadigm(parsed) {
...
export function buildGreekAdjParadigm(parsed) {
...
export function buildGreekVerbParadigm(parsed) {
...
export function buildGreekIrregularParadigm(parsed) {
