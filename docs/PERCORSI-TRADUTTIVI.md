...
| 12 | ✅ **FATTO** (`1bcac4e`) — (a) **stemma a barre trasversale**: la vista d'ingresso la dichiara la scheda (`stemmaDefault`) su tutte le superfici, non piu' una costante (ed era perfino forzata a `classic` nelle proposizioni); resta un default, il commutatore vive. (b) **«Leggere tanto»** v1: l'ingrediente mancante non era una tappa ma il fatto che lo **scheletro ignorava l'asse `vista`** — ora rispetta «frase corrente» e nasce il ciclo stretto, col passo «Frase 1 di 2»; ne guadagna anche il telefono (l'ordo era la fase piu' pesante). (c) **«Versione d'autore»**: tappe **condizionali** (`condizione`) — `participi` ◈, `ancore` ⚓, `attributivi` ⌖ — che esistono solo se il brano le chiede, e quindi non compaiono nemmeno nel nastro ne' nel conto. **Concatenamento verificato**: dichiarando «Assoluto» su un participio, la tappa «Agganci» entra da se' nella playlist. Piu' **tre card, non sei** all'avvio (`primario` sulla scheda + «Altri percorsi»), come l'audit prescriveva contro il choice overload | basso/nuovo | **Due difetti miei trovati collaudando**: leggevo `participioValore` (il nome della LISTA di opzioni) invece di `valoreParticipio` (il campo della voce); e usavo `suggerisciCapoAttributivo` come se restituisse il capo — restituisce `{capo, incastonato}` — accettando anche la semplice adiacenza, cosi' che perfino il soggetto risultava «incastonato». Corretto anche l'accordo di numero nei cartigli |
